  ...
  const fetchGithubRepos = async () => {
    ...
    try {
      const res = await fetch(
        'https://api.github.com/users/buddy0630/repos?sort=updated&per_page=12'
      );
    ...
    >
      ...
      <div className="relative z-20 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-6 items-start md:items-center justify-between mb-12">
          ...
          >
            <FaGithub size={20} />
            {isLoading ? 'Loading...' : 'Refresh GitHub Projects'}
          </motion.button>
        </div>
            ...
            >
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs bg-pink-500/20 text-pink-300 py-1 px-2 rounded-full">{project.date}</span>
                </div>
                <p className="text-gray-300 mb-4 line-clamp-3">{project.description}</p>
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.tags.map((tag, idx) => (
                    <span key={idx} className="text-xs bg-gray-700 text-gray-300 py-1px px-2 rounded-md">{tag}</span>
                  ))}
                </div>
                <div className="flex justify-end gap-3">
                  {project.githubUrl && (
                    <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="p-2 text-gray-400 hover:text-white transition-colors">
                      <FaGithub className="text-xl" />
                    </a>
                  )}
                  {project.liveUrl && (
                    <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="p-2 text-gray-400 hover:text-white transition-colors">
          ...
          <motion.div className="mt-16" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {githubRepos.map((repo) => (
                <motion.div key={repo.id} className="bg-gray-800/50 p-5 rounded-lg border border-gray-700" variants={cardVariants}>
                  <h4 className="text-lg font-semibold mb-2">{repo.name}</h4>
                  <p className="text-gray-300 text-sm mb-3 line-clamp-2">{repo.description || 'No description available'}</p>
                  <div className="flex justify-between items-center">
                    <div className="flex gap-3">
                      <span className="text-xs flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-yellow-400" />{repo.language || 'N/A'}</span>
                    ...
                    <a href={repo.html_url} target="_blank" rel="noopener noreferrer" className="text-pink-400 hover:text-pink-300 text-sm">View</a>
                  </div>
                </motion.div>
              ))}
            </div>
